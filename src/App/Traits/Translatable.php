...
use Revys\Revy\App\Exception\LocalesNotDefinedException;
...
trait Translatable
{
    ...
    public function getTranslationModelNameDefault()
    {
        ...
        return get_class($this).$config->get('revy.translatable.translation_suffix', 'Translation');
    }
    /**
     * @return string
     */
    ...
    {
        $config = app()->make('config');
        return $this->localeKey ?: $config->get('revy.translatable.locale_key', 'locale');
    }
    /**
    ...
    {
        return app()->make('config')->get('revy.translatable.use_property_fallback', false);
    }
    /**
    ...
    private function getFallbackLocale($locale = null)
    {
        ...
        foreach ($locales as $code) {
            if ($this->getTranslationByLocaleKey($code) !== null)
                return $code;
        }

        return app()->make('config')->get('revy.translatable.fallback_locale');
    }
    // private function getFallbackLocale($locale = null)
    // {
    //     if ($locale && $this->isLocaleCountryBased($locale)) {
    ...
    private function useFallback()
    {
        ...
        return app()->make('config')->get('revy.translatable.use_fallback');
    }
    /**
     * @param string $key
     *
     ...
     * @param string $key
     *
     * @throws \Revys\Revy\App\Exception\LocalesNotDefinedException
     * @return bool
     */
    ...
    // protected function getLocales()
    // {
    //     $localesConfig = (array) app()->make('config')->get('revy.translatable.locales');
    ...
    {
        return app()->make('config')->get('revy.translatable.locale_separator', '-');
    }
    /**
     * @return bool
     */
    ...
    protected function locale()
    {
        ...
        return app()->make('config')->get('revy.translatable.locale')
    ...
    {
        return app()->make('config')->get('revy.translatable.to_array_always_loads_translations', true);
    }
}